...
const LEAGUE: LeagueConfig<'static> = LeagueConfig {
    competition: "elitettan",
    base_url: "www.elitettan.se",
    source_prefix: "elitettan",
    article_source_url: Some("https://www.elitettan.se/"),
};
...
mod tests {
    ...
    #[test]
    fn parses_elitettan_graphql_fixture() {
        let input = include_str!("../../tests/fixtures/elitettan_graphql.json");
        ...
        assert!(events.iter().any(|event| {
            event.title == "Mallbackens IF vs Jitex BK"
                && event.round_label.as_deref() == Some("Round 2")
        }));
    }
}
